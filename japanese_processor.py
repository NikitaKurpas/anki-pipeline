...
@dataclass
class JapaneseWordData(BaseWordData):
    reading: str
    ruby: str | None = None
    ...
    def to_str(self) -> str:
        return f'{self.word}:{self.meaning}:{self.reading}:{self.ruby}:{self.sentence}:{self.sentence_meaning}:{self.sentence_ruby}:{self.notes}:{self.kanji_meaning}'
...
""".strip(),
        ...
        all_words = [
            JapaneseWordData(
                word=p[0].strip(), reading=p[1].strip(), meaning=p[2].strip()
            )
    ...
    async def enrich_words(
        ...
        for word in words.values():
            if not isinstance(word, JapaneseWordData):
                raise TypeError(f'Expected JapaneseWordData, got {type(word)}')
        words = cast(dict[str, JapaneseWordData], words)

        await self._generate_ruby(client, words)
        await self._generate_word_info(client, words)
        await self._generate_example_sentences(client, words)
        await self._generate_sentence_ruby(client, words)

    async def _generate_ruby(
        self, client: AsyncOpenAI, words: dict[str, JapaneseWordData]
    ) -> None:
        instructions = """
...
"""
        response = await client.chat.completions.create(
            ...
            messages=[
                {'role': 'system', 'content': instructions},
                {
                    'role': 'user',
                    'content': '\n'.join(
                        [f'{w.word}:{w.reading}' for w in words.values()]
                    ),
                },
            ],
        )
        ...
        if len(valid_lines) != len(words):
            raise GenerationError(
                f'Expected {len(words)} lines, got {len(valid_lines)}'
            )
...
"""
    ...
    async def _generate_example_sentences(
        self, client: AsyncOpenAI, words: dict[str, JapaneseWordData]
    ) -> None:
        instructions = """
...
"""
    ...
    async def _generate_sentence_ruby(
        self, client: AsyncOpenAI, words: dict[str, JapaneseWordData]
    ) -> None:
        instructions = """
...
"""
    ...
    def get_note_fields(self, word_data: BaseWordData) -> dict[str, str]:
        if not isinstance(word_data, JapaneseWordData):
            raise ValueError(f'Expected JapaneseWordData, got {type(word_data)}')

        return {
            'Word': word_data.word,
            'Word Meaning': word_data.meaning or '',
            'Word Reading': word_data.reading or '',
            'Word Furigana': word_data.ruby or '',
            'Sentence': word_data.sentence or '',
            'Sentence Meaning': word_data.sentence_meaning or '',
            'Sentence Furigana': word_data.sentence_ruby or '',
            'Notes': word_data.notes or '',
            'Kanji Meaning': word_data.kanji_meaning or '',
        }

    def get_note_model(self) -> str:
        return 'Kaishi Alt Vocab'
